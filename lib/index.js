...
_cleanStaticClientAssets = function(mimosaConfig, options, next) {
  clientAssets.map(function(clientAsset) {
    var fileName;
    fileName = path.basename(clientAsset);
    return path.join(clientFolder, fileName);
  }).filter(function(clientAsset) {
    return fs.existsSync(clientAsset);
  }).forEach(function(clientAsset) {
    logger.debug("Removing mimosa-testem-require asset [[ " + clientAsset + " ]] removed");
    return fs.unlinkSync(clientAsset);
  });
  return next();
};
...
_buildRequireConfig = function(mimosaConfig, options, next) {
  ...
  if (requireConfig.shim != null) {
    if (requireConfig.shim['testem-require'] == null) {
      requireConfig.shim['testem-require/mocha'] = {
        exports: 'mocha'
      };
    }
  } else {
...
__writeAssets = function(safeAssets, assets, folder) {
  return assets.filter(function(asset) {
    return safeAssets.indexOf(path.basename(asset)) === -1;
